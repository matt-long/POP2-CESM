...
! QL, 150526, LAMULT, USTOKES and VSTOKES
   ...
   call POP_HaloUpdate(ATM_XTFE_FLUX,POP_haloClinic, &
                       POP_gridHorzLocCenter,           &
                       POP_fieldKindScalar, errorCode,  &
                       fillValue = 0.0_POP_r8)
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'update_ghost_cells_coupler: error updating ATM_XTFE_FLUX')
      return
   endif

   call POP_HaloUpdate(SEAICE_XTFE_FLUX,POP_haloClinic, &
                       POP_gridHorzLocCenter,           &
                       POP_fieldKindScalar, errorCode,  &
                       fillValue = 0.0_POP_r8)
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'update_ghost_cells_coupler: error updating SEAICE_XTFE_FLUX')
      return
   endif
